...
This simple assembler assumes an ISA containing the following instructions: loadi, mov, add, sub, and, or, j, beq, bne, lwi, swd, swi. 
All instructions are encoded into 32-bit words based on the following format:

Bits 31-24 : OP-CODE				: Given as one of (loadi, mov, add, sub, and, or, j, beq, bne, lwi, swd, swi)
 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*******************************************************************/
		...
		while(in_token!=NULL)
		{
			...
			else if(strcmp(in_token,"8")==0 || strcmp(in_token,"8\n")==0) strcpy(out_token, "00001000");
			else if(strcmp(in_token,"9")==0 || strcmp(in_token,"9\n")==0) strcpy(out_token, "00001001");
			else if(strcmp(in_token,"10")==0 || strcmp(in_token,"10\n")==0) strcpy(out_token, "00001010");
			else if(strcmp(in_token,"11")==0 || strcmp(in_token,"11\n")==0) strcpy(out_token, "00001011");
			else if(strcmp(in_token,"12")==0 || strcmp(in_token,"12\n")==0) strcpy(out_token, "00001100");
			else if(strcmp(in_token,"13")==0 || strcmp(in_token,"13\n")==0) strcpy(out_token, "00001101");
